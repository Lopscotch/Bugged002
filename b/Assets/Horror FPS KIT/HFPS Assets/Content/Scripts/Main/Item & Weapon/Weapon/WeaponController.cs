...
public class WeaponController : MonoBehaviour, ISwitcher, ISwitcherWallHit, ISaveableArmsItem
{
    ...
    void Awake()
    {
        ...
        wepCamera = ScriptManager.Instance.ArmsCamera;
    ...
    void Update()
    {
        ...
        if (muzzleShow)
        {
            Vector3 muzzleRotation = muzzleFlash.transform.localEulerAngles;

            if (muzzleRot == Axis.X)
            {
                muzzleRotation.x = Random.value * 360;
            }
            else if (muzzleRot == Axis.Y)
            {
                muzzleRotation.y = Random.value * 360;
            }
            else if (muzzleRot == Axis.Z)
            {
                muzzleRotation.z = Random.value * 360;
            }

            muzzleFlash.transform.localEulerAngles = muzzleRotation;
    ...
    {
        bulletsRemaining = (int)token["bulletsInMag"];
        bullets = (int)token["allBullets"];
