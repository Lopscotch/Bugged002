...
public class InventoryContainer : MonoBehaviour, ISaveable
{
    ...
    public void TakeBack(bool all)
    {
        if (inventory.CheckInventorySpace())
        {
            ...
            if (all)
            {
                inventory.AddItem(lastSelectedItem.item.ID, lastSelectedItem.amount, lastSelectedItem.customData);
                RemoveItem(lastSelectedItem, true);
                Destroy(destroyObj);
            }
            else
            {
                if (lastSelectedItem.item.itemType != ItemType.Weapon && lastSelectedItem.item.itemType != ItemType.Bullets)
                {
                    if (lastSelectedItem.amount == 1)
                    {
                        Destroy(destroyObj);
                    }

                    inventory.AddItem(lastSelectedItem.item.ID, 1, lastSelectedItem.customData);
                    RemoveItem(lastSelectedItem, false);
                }
                else
                {
                    inventory.AddItem(lastSelectedItem.item.ID, lastSelectedItem.amount, lastSelectedItem.customData);
